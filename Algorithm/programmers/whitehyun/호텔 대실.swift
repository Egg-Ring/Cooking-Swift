...
struct Number155651: Solution {
  ...
  mutating func solution(_ bookTime: [[String]]) -> Int {
    ...
    sortedBookingTime[1...].forEach { time in
      let startTime = time[0] // 시작 시간
      let endTime = time[1]   // 끝나는 시간
      
      // 큐에 청소가 끝난 방이 있는지 확인
      let cleanedRooms = queue.enumerated().filter { index, cleaningDeadline in
        return cleaningDeadline <= startTime
      }
...
extension Number155651 {
  private struct DateTime: Comparable, CustomStringConvertible {
    ...
    static func minutes(_ value: Int) -> DateTime {
      return DateTime(hour: value / 60, minute: value % 60)
    }
